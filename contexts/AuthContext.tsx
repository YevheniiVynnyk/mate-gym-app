...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
    ...
    const initAuth = async () => {
      ...
      if (token) {
        try {
          ...
          const hasAcceptedTerms = await tosService.checkAccepted(
            mappedUser.id,
          );
          ...
          await AsyncStorage.setItem(
            "fittracker_user",
            JSON.stringify(userWithTermsStatus),
          );
        } catch (error) {
  ...
  const register = async (
    ...
    setIsLoading(true);
    try {
      const tokenData = await authService.signUp({
        login,
        password,
        email,
        role,
  ...
  const handleAuthSuccess = async (tokenData: any) => {
    ...
    await AsyncStorage.setItem(
      "fittracker_user",
      JSON.stringify(userWithFirstLogin),
    );
  };
