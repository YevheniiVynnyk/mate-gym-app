...
import {TrainingDayCreateDTO, TrainingDayDTO} from '@/services/trainingDayService.ts';

export const mapToCreateDTO = (localData: TrainingDay): TrainingDayCreateDTO => {
	return {
		name: localData.name,
		date: localData.date,
...
export const mapFromAPI = (apiData: TrainingDayDTO): TrainingDay => {
	return {
		...
		trainings: apiData.trainings.map(training => ({
			...
			trainingDetails: training.trainingDetails.map((trainingDetail, index) => ({
				id: trainingDetail.id,
				set: index + 1,
				weight: trainingDetail.weight,
				repetition: trainingDetail.repetition,
