...
export default function TrainingExercisesList({ trainingDay }: Props) {
    return (
        <ScrollView
            className="bg-white p-2 my-2 rounded-2xl" // можно менять высоту при необходимости
            showsVerticalScrollIndicator={false}
        >
            {trainingDay.trainings.map((training: any, i: number) => (
                <View key={training.id} className="rounded-xl p-2 mb-4 border border-gray-100 ">
