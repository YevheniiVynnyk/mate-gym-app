...
const PlanCard: React.FC<PlanCardProps> = ({plan, onPurchase, isPurchased = false}) => {
	...
	return (
		<View className={`bg-white rounded-xl p-4 mb-4 shadow ${plan.popular ? "border-2 border-blue-400" : ""}`}>
			{plan.popular && (
				<View
					className="absolute -top-2 left-1/2 transform -translate-x-1/2 flex-row items-center bg-blue-400 px-3 py-1 rounded-full">
			...
			>
				<Text
					className={`font-semibold ${isPurchased ? "text-gray-600" : "text-white"}`}>{isPurchased ? "Уже куплено" : "Купить план"}</Text>
			</TouchableOpacity>
		</View>
	);
};
