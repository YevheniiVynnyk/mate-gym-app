...
export const DialogFooter: React.FC<{ children: React.ReactNode; className?: string }> = ({
    children,
    className,
}) => <View className={cn('flex flex-row justify-end space-x-2 mt-4', className)}>{children}</View>;
...
export const DialogContent: React.FC<{ children: React.ReactNode; className?: string }> = ({
    children,
    className,
}) => <ScrollView className={cn('max-h-[80%] w-full', className)}>{children}</ScrollView>;
