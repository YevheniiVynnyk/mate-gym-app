...
const CardHeader = React.forwardRef<View, ViewProps>((props, ref) => {
    const { className, ...rest } = props;
    return (
        <View ref={ref} className={`flex flex-col space-y-1.5 p-4 ${className || ''}`} {...rest} />
    );
});
...
const CardDescription = React.forwardRef<Text, TextProps>((props, ref) => {
    const { className, ...rest } = props;
    return (
        <Text ref={ref} className={`text-sm text-muted-foreground ${className || ''}`} {...rest} />
    );
});
...
const CardContent = React.forwardRef<View, ViewProps>((props, ref) => {
    const { className, ...rest } = props;
    return <View ref={ref} className={`p-4 pt-0 ${className || ''}`} {...rest} />;
});
