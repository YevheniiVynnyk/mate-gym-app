...
import { Training, TrainingDay, Exercise } from "@/types/trainingDay";
...
export const useTrainingDayForm = (isEdit?: boolean) => {
	...
	const saveTrainingDay = async (execute = false, duration?: number) => {
		let finalName = trainingDayName.trim() || `Тренировка ${trainingDayDate.toLocaleDateString()}`;
		...
		try {
			...
			if (clientId)
				await trainingDayService.createTrainingForClient(+clientId, mapToCreateDTO(payload));
