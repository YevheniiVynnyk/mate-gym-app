...
export const updateTrainingField = <K extends keyof Training>(
    ...
    field: K,
    value: Training[K],
): Training[] => trainings.map((t) => (t.id === trainingId ? { ...t, [field]: value } : t));
...
export const updateSetsCount = (trainings: Training[], trainingId: number, sets: number) =>
    trainings.map((t) => {
        if (t.id === trainingId) {
            const updatedDetails = Array.from(
                { length: sets },
                (_, i) =>
                    t.trainingDetails[i] || { id: null, set: i + 1, weight: 0, repetition: 0 },
            );
            return { ...t, trainingDetails: updatedDetails };
        }
        return t;
...
export const calculateTotals = (details: TrainingDetail[]) => {
    ...
    const totalWeight = details.reduce((sum, d) => sum + d.repetition * d.weight, 0);
